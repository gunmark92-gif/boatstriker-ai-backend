...
- 「■あなたの見送り傾向」の項目がある場合は必ず言及すること。これはユーザー自身の実際の購入/見送り行動パターンであり、的中率とは別の判断材料。「今回はいつもの傾向と合っている/外れている」という視点を一言加えること（例：「この会場は普段64%見送っているパターンです。今回もその通り見送るか、あえて踏み込むかはあなた次第です」）。ユーザーの行動を否定せず、あくまで「鏡」として映すに留める
- 断定しすぎず、根拠になっている数値・過去実績を必ず引用すること
- 日本語で400字以内（見送り傾向への言及を含めてもこの範囲に収めること。長くなりそうな場合は箇条書きの数を絞って対応する）
  ...
  try {
    const apiRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1200, // 2026/9/19：700だと「あなたの見送り傾向」追加後に出力が途中で切れる事例が発生したため引き上げ
